...
module.exports = async (client, reaction, user) => {
  ...
  if (ticketDataByHeaderId) {
    if (ticketDataByHeaderId.ticket_header_message_id === reaction.message.id) {
      ...
      if (reaction.emoji.name === "🛑") {
        if (ticketDataByHeaderId.resolved === 0) {
          ...
          resolvedEmbed.setDescription(
            `Le ticket #${ticketId} est maintenant fermé.\nOuvre en un nouveau si tu as besoin.\n\n*__Note:__ Tu auras toujours accès à ce ticket tant qu'un admin ou toi ne le supprime pas via le tableau de bord épinglé.*`
          );
